...
public class BanCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if(sender instanceof ProxiedPlayer){
            ...
            if(player.hasPermission("rcbproxy.ban")){
                if (args.length < 3) {
                    helpMessage(sender);
                    return;
                }
                ...
                if (args[1].equalsIgnoreCase("perm")) {
                    RcbProxy.getInstance().getBanManager().ban(targetUUID, -1L, reason, player.getName());
                ...
                RcbProxy.getInstance().getBanManager().ban(targetUUID, banTime, reason, player.getName());
